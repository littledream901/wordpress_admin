...
3. 拉取最新代码
4. **交互式确认版本号**（见下方说明）
5. 构建新镜像（老容器继续运行，用户不中断）
6. 优雅停止旧容器 → 启动新容器（中断仅 3-5 秒）
7. 等待健康检查通过
8. 清理旧 Docker 镜像

### 版本号与菜单同步

系统用 `VERSION` 作为初始化标记：Config 表里存了上次初始化的版本号，如果和当前 `VERSION` 一致，启动时会**跳过**菜单/API/角色同步。因此新增菜单或接口后，必须变更版本号才能让改动落库。

`update` 执行到第 4 步时会提示：

```
当前版本: 0.1.1
提示: 修改版本号会触发菜单/API/角色的重新同步
      格式建议: v1.0.1 或 0.1.2 或延续当前版本号

请输入新版本号（直接回车保持 0.1.1）:
```

- **本次更新包含菜单/接口变更** → 输入新版本号，如 `0.1.2`
- **仅修改业务逻辑，菜单没动** → 直接回车跳过

版本号写入 `.env` 的 `VERSION=`，优先级高于 `app/settings/config.py` 里的默认值，所以不会产生代码改动，`git pull` 也不会冲突。

**非交互场景**（CI/CD 或无 TTY）：

```bash
# 预设版本号
NEW_VERSION=0.1.2 bash deploy/deploy.sh update

# 无 TTY 且未预设时，自动保持当前版本号，不会阻塞等待输入
```
...
```bash
# 改 .env 后重启即可
sed -i "s/^VERSION=.*/VERSION=0.1.2/" .env
