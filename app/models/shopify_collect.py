...
class ShopifySource(BaseModel, TimestampMixin):
    source_url = fields.CharField(max_length=500, unique=True, description='待采集URL', index=True)
    source_type = fields.CharField(max_length=32, default='collection', description='source类型：collection/product')
    status = fields.CharField(max_length=64, default='pending', description='采集状态', index=True)
...
class ShopifyProduct(BaseModel, TimestampMixin):
    source_id = fields.IntField(description='来源ID', index=True)
    source_url = fields.CharField(max_length=500, default='', description='来源URL')
    product_url = fields.CharField(max_length=500, unique=True, description='产品URL', index=True)
    handle = fields.CharField(max_length=255, default='', description='handle', index=True)
    ...
    status = fields.CharField(max_length=64, default='ready', description='状态', index=True)
    prod_info_json = fields.TextField(default='{}', description='完整产品JSON')
    assigned_site_id = fields.IntField(null=True, description='分配目标站点ID', index=True)
    assigned_status = fields.CharField(max_length=64, default='', description='分配状态', index=True)
    imported_site_id = fields.IntField(null=True, description='最近导入站点ID', index=True)
    imported_status = fields.CharField(max_length=64, default='', description='导入状态', index=True)
