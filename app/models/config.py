...
class Config(BaseModel, TimestampMixin):
    ...
    name = fields.CharField(max_length=100, description="配置键名", index=True)
    value = fields.TextField(default="", description="配置值")
    description = fields.CharField(max_length=255, null=True, description="配置说明")
    category = fields.CharField(max_length=50, choices=CATEGORY_CHOICES, description="配置分类", index=True)
