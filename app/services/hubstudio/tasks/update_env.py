...
def execute_update_env(executor, job: dict, payload: dict) -> dict:
    ...
    if remark:
        try:
            # 先查询当前环境名称（避免修改环境名）
            # 尝试按 containerCode 精确查询（如果 API 支持）
            env_list = client.get_env_list(current=1, size=200, containerCode=int(hub_env_id))
            current_env = None
            ...
            # 如果精确查询失败，降级使用分页查询前 200 个
            if not current_env:
                env_list = client.get_env_list(current=1, size=200)
                if env_list.get("code") == 0 and env_list.get("data", {}).get("list"):
                    for env in env_list["data"]["list"]:
                        if str(env.get("containerCode")) == str(hub_env_id):
                            current_env = env
            ...
            if current_env:
                ...
                executor.logger.warning(f"[update_env] 未找到环境 {hub_env_id}，使用备用名称: {container_name}")
