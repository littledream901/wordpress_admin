<template>
  <CommonPage title="Outlook 管理">
    <template #action>
      <n-space>
        <n-button v-permission="'post/api/v1/outlook/create'" type="primary" @click="handleAdd">新增 Outlook</n-button>
        <input ref="fileInputEl" type="file" accept=".xlsx,.csv" style="display:none" @change="handleFileChange" />
        <n-button v-permission="'post/api/v1/outlook/batch-create'" @click="fileInputEl.click()" :loading="uploadLoading">导入 Excel</n-button>
      </n-space>
    </template>
    ...
    <CrudTable ref="$table" v-model:query-items="queryItems" :get-data="api.getList" :columns="columns" :pagination="pagination" @on-checked="onCheckedChange">
      ...
      <template #queryBarActions>
        <template v-if="checkedRowKeys.length">
          ...
          <n-button v-permission="'post/api/v1/outlook/batch-delete'" type="error" @click="showBatchDeleteConfirm = true">批量删除</n-button>
    ...
    <n-modal v-model:show="showBatchDeleteConfirm" preset="dialog" title="确认批量删除"
      ...
      确定要删除 <b>{{ checkedRowKeys.length }}</b> 个 Outlook 账号吗？此操作不可撤销。
    </n-modal>
  </CommonPage>
</template>
<script setup>
...
async function handleFileChange(e) {
  ...
  try {
    ...
    const res = await importJobApi.upload('outlook', formData)
    const data = res?.data ?? {}
