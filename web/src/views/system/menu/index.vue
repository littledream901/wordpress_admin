...
async function handleDelete(row) {
  ...
  if (childCount > 0) {
    // 有子菜单 → 级联删除
    try {
      await api.deleteMenuCascade({ id: row.id })
      window.$message?.success(`已级联删除该菜单及其 ${childCount} 个子菜单`)
        ...
        h(
          ...
          },
          {
            ...
            default: () => {
              const childCount = countDescendants(row)
              if (childCount > 0) {
                return h('div', { style: 'color:#e63946' }, `该菜单下有 ${childCount} 个子孙菜单，确定级联删除吗？`)
              }
              return h('div', {}, '确定删除该菜单吗?')
            },
          }
        ),
      ]
      ...
      >
        ...
        >
          <NInput
            ...
            :placeholder="modalForm.parent_id > 0 ? '相对路径，如 monitor、user' : '绝对路径，如 /system、/onepanel-monitor'"
          />
        </NFormItem>
        <NFormItem
          v-if="modalForm.menu_type === 'menu'"
          label="组件路径"
          path="component"
          :rule="{
            required: true,
            message: '菜单必须设置组件路径，否则无法加载页面',
            trigger: ['blur'],
          }"
        >
          <NInput
            v-model:value="modalForm.component"
            placeholder="以 / 开头，如 /system/user、/site-pipeline/site-list"
          />
        </NFormItem>
        <NFormItem
          label="跳转路径"
          path="redirect"
          :rule="modalForm.menu_type === 'catalog' && modalForm.parent_id === 0 ? {
            required: true,
            message: '根目录菜单必须设置跳转路径（留空则点击目录无响应）',
            trigger: ['blur'],
          } : {}"
        >
          <NInput
            v-model:value="modalForm.redirect"
            :placeholder="modalForm.parent_id === 0 ? '以 / 开头，如 /system/user' : '仅根目录可设置'"
