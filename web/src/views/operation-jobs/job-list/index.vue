<template>
  <CommonPage show-header title="任务列表">
    ...
    <n-modal v-model:show="showDetail" title="任务详情" preset="dialog">
      <n-descriptions v-if="detail" label-placement="left" :column="1" bordered>
        ...
        <n-descriptions-item label="步骤">{{ detail.step }}/{{ detail.total_steps }}</n-descriptions-item>
        <n-descriptions-item label="批次">{{ detail.batch_id }}</n-descriptions-item>
        <n-descriptions-item label="节点">{{ detail.worker_name }}</n-descriptions-item>
        <n-descriptions-item label="账号"><n-tag type="info" size="small">{{ providerName }}</n-tag></n-descriptions-item>
        <n-descriptions-item label="错误"><span style="color:red">{{ detail.error_message }}</span></n-descriptions-item>
        <n-descriptions-item label="结果"><pre style="max-height:200px;overflow:auto;font-size:12px">{{ formatJson(detail.result_json) }}</pre></n-descriptions-item>
...
import { NButton, NDescriptions, NDescriptionsItem, NInput, NModal, NSelect, NTag } from 'naive-ui'
...
const actionOptions = [
  ...
  { value: 'hub_website_control', label: 'Hub网站控制' },
...
const columns = [
  ...
  { title: '操作', key: 'action_type', width: 120 },
